...
public class LinesSerializer extends JsonSerializer<Lines> {
	...
	@Override
	public void serialize(Lines value, JsonGenerator jgen, SerializerProvider provider)
			throws IOException, JsonProcessingException {
		try {
			jgen.writeStartArray();

			List<Serializable> list = value.getLineRefOrLineDirection();
			for (Object object : list) {
				...
				if (object instanceof LineRefStructure) {
					...
					if (lineDirection.getLineRef() != null) {
						jgen.writeStringField("LineRef", lineDirection.getLineRef().getValue());
					}
					if (lineDirection.getDirectionRef() != null) {
						jgen.writeStringField("DirectionRef", lineDirection.getDirectionRef().getValue());
					}
